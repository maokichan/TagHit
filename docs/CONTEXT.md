...
已完成（细节见 DECISIONS 与 git log）：契约 v0 ✅ 旧 UI 吸收 ✅ 贡献点 v0 ✅ 真机接线 ✅ 字节闸门 ✅ 界面美化 ✅ 图片尺寸落库 ✅ 注册表 v0.1 ✅ 命令注册表 + 右键菜单闭环 ✅ 插件生态主线（SurfaceHost 标准容器 + 服务面 dialog/toast + config 仓 featureId:key + contentTab 槽 + HostApi 冻结面 + contributed 装载器，2026-09-07）✅ **视频缩略图 + 多选批量（D16，2026-09-07）** ✅ **无边框窗口 + 窗口控制键 + 去默认菜单 + 主色去蓝（D17，2026-09-12）** ✅。

1. **标准功能组件与详情界面翻新（2026-09-12 指定）**：浏览视图（显示/item 网格）、tag 面板、来源管理面板、条目详情界面——均为旧 UI 吸收期实现，未随 0.2.x 机制与视觉演进，整体更新。
2. **三方插件发现/分发**：机制（声明/装载/隔离/权限面）已就绪；目录扫描、安装、启停管理待分发形态裁决。
3. Backlog：**tag 修饰（未讨论：语义与形态均未裁决，parked 记名，不预建模）**、**条目拖出应用得到 taghit-file:// 协议地址（应交付真实文件路径；系统间拖放语义，2026-09-12 记录）**、事件（D6，扫描进度→进度 UI）、EAV 元数据建模、标签语义带出、config 持久化迁宿主（存储键形状不变）、TEXT_EXTS 双表收敛（若白名单频繁变动则契约加拉取端点）、StartScreen 内联搜索收敛到 globalSearch 功能组件、多选进阶（Shift 区间选择/拖拽）、封面提取（缩略图落库已就绪，可升级为自动截取首帧之外的封面）、LICENSE、CI。
