...
1. **标准功能组件与详情界面翻新（2026-09-12 指定）**：~~浏览视图、tag 面板、来源管理~~ **来源根树容器已落地（0.2.11 后）**；剩余：浏览视图（item 网格）、tag 面板、条目详情界面，及各功能组件的专属全页内容页（角标暂全页复用窄面板）。
2. **三方插件发现/分发**：机制（声明/装载/隔离/权限面）已就绪；目录扫描、安装、启停管理待分发形态裁决。
3. Backlog：**节点右键可见性操作（节点行右键菜单：隐藏该节点全部子条目 / 仅显示该节点或其子条目；依赖 MenuContext 扩展 node 目标 + 键鼠交互框架，2026-09-12 裁决：框架就位后再开发；语义基线：可见性 = 当前工作区浏览/排序/搜索的成员前提（browseWorkspace 投影已如此），全局搜索跨工作区不 gate）**、**tag 修饰（未讨论：语义与形态均未裁决，parked 记名，不预建模）**、**条目拖出应用得到 taghit-file:// 协议地址（应交付真实文件路径；系统间拖放语义，2026-09-12 记录）**、事件（D6，扫描进度→进度 UI）、EAV 元数据建模、标签语义带出、config 持久化迁宿主（存储键形状不变）、TEXT_EXTS 双表收敛（若白名单频繁变动则契约加拉取端点）、StartScreen 内联搜索收敛到 globalSearch 功能组件、多选进阶（Shift 区间选择/拖拽）、封面提取（缩略图落库已就绪，可升级为自动截取首帧之外的封面）、文件管理组件进阶（新建/目录操作；移动目标仍手输，可复用 dialog.pickDirectory）、LICENSE、CI。
