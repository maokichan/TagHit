...
- 注册表 = 声明表（2026-09-07）：可序列化 FeatureManifest + 按来源分层的实现绑定（official 直连 / contributed loader，类型先行）分离；错误隔离双线（setup try/catch + 槽渲染 FeatureBoundary），官方组件同边界；生命周期 per-entry（setup/dispose 配对 + unregisterFeature）。
