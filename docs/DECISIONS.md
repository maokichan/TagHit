...
| D18 | 文件身份与来源根树/文件管理（2026-09-12） | **扫描认领**：新路径与 missing 条目同 contentHash（且旧路径确已消失）→ 原条目改写 sourceUri 恢复 active（itemsRelocated；id 不变，标签/作品成员随行）——移动语义，改名/移动经重扫收敛不丢标签。**文件操作闸门**：fs.move/fs.trash 只收来源根之下路径（路径段匹配，与字节闸门 D15 同规则）；删除走 Trash 端口（宿主注入 shell.trashItem 进回收站），不可恢复删除不进用例面。**路径归一化在边界执行**（2026-09-12 补）：normalizePath（反斜杠→正斜杠、去尾分隔符）应用于挂载/卸载、节点与文件操作用例入口、NodeFileSystem.walk 基准——用户输入/原生选择器的混合分隔符不得产生双身份节点。**dialog.pickDirectory**：原生目录选择是唯一 dialog 端点（不开通用口子）。**来源根树**：每根一个目录树容器（分割线分离，不用圆角容器），树形由路径前缀派生渲染（不另存父指针，与域模型一致）；可见性**含根节点在内**逐节点可切换、不级联（域现状），Shift+点击 = 子树批量入口（应用层单事务逐节点，非级联语义）。**停靠面板角标**：SurfaceHost 活动栏面右下角角标 → openFeature 内容标签页，全页暂复用窄面板实现（翻新项）。**渲染进程崩溃自恢复**：render-process-gone 记日志 + 重载（上限 3 次，防循环） |

## parked

C-S · 作品嵌套/多归属 · 后台增量扫描（子树整体排除已落地为应用层批量入口 setSubtreeState，D18；待办细化见根目录 TODO.md）
